...
class Page(object):
    ...
    def save(self, update=True):
        ...
        if not os.path.exists(folder):
            os.makedirs(folder)
        #       makes an archive only if page exists already
    ...
    def archive(self, folder):
        ...
        Copies file before edit into the "revised/url" folder
        and gives it a unique name based on the current time
        folder will be created if it's the first edit and will persist
        even if the title is changed

        """
        if not os.path.exists(folder + "/revised/"):
            os.makedirs(folder + "/revised/")
        revised_folder = folder + "/revised/" + self.url
        timestr = datetime.now().strftime("%Y%m%d-%H%M%S")
        x = datetime.now()
        if not os.path.exists(revised_folder):
            os.makedirs(revised_folder)
        copy2(self.path, revised_folder)
        new_file = revised_folder + "/" + self.url + "_" + timestr + ".md"
        os.rename(revised_folder + "/" + self.url + ".md", new_file)
        with open(new_file, 'r+', encoding='utf-8') as f:
            data = f.read()
            f.seek(data.index("date: "))
            f.write("\ndate: " + x.strftime("%c") + '\n')
    ...
    # Handles new meta data tag "date"
    @property
    def date(self):
        try:
            return self['date']
...
class Wiki(object):
    ...
    def index(self):
        ...
        for cur_dir, _, files in os.walk(root):
            ...
            for cur_file in files:
                ...
                if cur_file.endswith('.md') and not cur_dir.__contains__("revised"):
    ...
    def revisions(self, url):
        """
        Similar to index, builds a list of all the available revisions of a page
        ...
        pages = []
        root = os.path.abspath(self.root)
        for cur_dir, _, files in os.walk(root + "/revised/" + url):
            # get the url of the current directory
            cur_dir_url = cur_dir[len(root)+1:]
            for cur_file in files:
                path = os.path.join(cur_dir, cur_file)
