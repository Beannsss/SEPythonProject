...
WIKILINK_PAGE_CONTENT = """\
title: link

[[target]]
"""

WIKILINK_CONTENT_HTML = """\
...
class WikilinkTestCase(TestCase):
    ...
    def test_simple_wikilink(self):
        ...
        """
        formatted = wikilink('[[target]]', simple_url_formatter)
    ...
    def test_titled_wikilink(self):
        ...
        """
        formatted = wikilink('[[target|Target]]', simple_url_formatter)
        assert formatted == "<a href='/target'>Target</a>"
    ...
    def test_multiple_wikilinks(self):
        ...
        """
        formatted = wikilink(
            '[[target|Target]] is better than [[alternative]]',
...
class ProcessorTestCase(WikiBaseTestCase):
    ...
    def test_process(self):
        ...
        assert original == PAGE_CONTENT.split('\n\n', 1)[1]
        assert meta == {
            'title': 'Test',
            'tags': 'one, two, 3, jö',
            'date': 'today'
...
class PageTestCase(WikiBaseTestCase):
    ...
    def setUp(self):
        ...
        self.page_relative = self.page_path.replace('test.md', '')
    ...
    def test_page_meta(self):
        ...
        """
        assert self.page.title == 'Test'
        assert self.page.tags == 'one, two, 3, jö'
    ...
    def test_page_archive(self):
        """
            Checks the before and after archive, Making sure the /revised/ folder and
            file are created, but did not exist before
        """
        self.assertEqual(os.path.exists(self.page_relative + "/revised/"), False)
        self.page.archive(self.page_relative)
        assert os.path.exists(self.page_relative + "/revised/")
        self.assertEqual(os.listdir(self.page_relative + "/revised/"), ['test'])
